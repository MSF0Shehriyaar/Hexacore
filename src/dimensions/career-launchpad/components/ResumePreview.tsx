...
interface ResumePreviewProps {
  resume: ResumeData;
  template: 'executive' | 'ats' | 'ats_with_image';
...
export default function ResumePreview({ resume, template, onUpdate, isCompact = false }: ResumePreviewProps) {
  ...
  useEffect(() => {
    let timeoutId: NodeJS.Timeout;
      ...
      // Pass 3: Container Height Hardening
      // Ensures that if content spills into Page 2, Page 2 is shown as a full A4 sheet
      // even if partially empty.
      const rect = container.getBoundingClientRect();
      const lastElement = elements[elements.length - 1];
      if (lastElement) {
        let lastBottom = 0;
        let curr: HTMLElement | null = lastElement;
        while (curr && curr !== container) { lastBottom += curr.offsetTop; curr = (curr.offsetParent as HTMLElement); }
        lastBottom += lastElement.offsetHeight;
        
        // Add padding bottom of the template
        const pages = Math.ceil((lastBottom + PAGE_MARGIN) / A4_HEIGHT) || 1;
        container.style.minHeight = `${pages * A4_HEIGHT}px`;
      }
    };
  ...
  const renderATSWithImage = () => (
    <div className={`p-[20mm] text-[#000000] font-sans leading-normal transition-all duration-500`}>
      <header className="mb-4 flex items-center gap-8 border-b-2 border-black pb-4">
        {resume.personalInfo.profileImage && (
  ...
  return (
    <div className="relative group/preview">
      ...
      >
        ...
        {template === 'ats_with_image' && renderATSWithImage()}
      </div>
